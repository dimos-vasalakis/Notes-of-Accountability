...
async def _make_user_with_task(db_session: AsyncSession, *, due_date: datetime, notified_at=None) -> Task:
    user = User(email=f"{id(due_date)}@example.com", hashed_password="hashed")
    db_session.add(user)
    ...
    task = Task(owner_id=user.id, title="Do the thing", due_date=due_date, notified_at=notified_at)
    db_session.add(task)
